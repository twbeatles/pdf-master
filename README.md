# PDF Master v4.5.6

📑 **올인원 PDF 편집 프로그램** — PyQt6 기반 데스크톱 앱

![Python](https://img.shields.io/badge/Python-3.10+-blue)
![PyQt6](https://img.shields.io/badge/PyQt6-6.5+-green)
![PyMuPDF](https://img.shields.io/badge/PyMuPDF-fitz-orange)
![License](https://img.shields.io/badge/License-MIT-yellow)

[🇺🇸 English README](README_EN.md) | [🇰🇷 한국어](#)

---

## 📋 목차

- [주요 기능](#-주요-기능)
- [설치 및 실행](#-설치-및-실행)
- [사용 방법](#-사용-방법)
- [단축키](#️-단축키)
- [알려진 제한사항](#️-알려진-제한사항)
- [빌드](#-빌드-pyinstaller)
- [변경 이력](#-변경-이력)

---

## ✨ 주요 기능

### 📄 PDF 병합 및 변환
| 기능 | 설명 | 지원 형식 |
|------|------|----------|
| **PDF 병합** | 여러 PDF를 하나로 합치기 | 드래그 앤 드롭 지원 |
| **PDF → 이미지** | 페이지별 이미지 변환 | PNG, JPG, WEBP, BMP, TIFF |
| **이미지 → PDF** | 여러 이미지를 PDF로 합치기 | PNG, JPG, BMP, GIF, WEBP |
| **텍스트 추출** | PDF에서 텍스트 추출 (선택: **OCR** — 스캔본, 시스템 Tesseract 필요) | TXT 저장 |

### ✂️ 페이지 편집
| 기능 | 설명 |
|------|------|
| **페이지 추출** | 원하는 페이지만 분리 (예: `1-3, 5, 7-10`) |
| **페이지 삭제** | 선택한 페이지 제거 |
| **페이지 회전** | 90°, 180°, 270° 회전 / 선택한 페이지만 부분 회전 |
| **페이지 순서 변경** | 드래그 앤 드롭으로 순서 재배치 |
| **페이지 번호 삽입** | 위치·형식·폰트 커스텀 (`Page 1 of N`, `1/N` 등) |
| **빈 페이지 삽입** | 원하는 위치에 빈 페이지 추가 |
| **페이지 복제** | 선택한 페이지 복사 |
| **페이지 역순 정렬** | 전체 페이지 순서 뒤집기 |
| **페이지 크기 변경** | A4, A3, Letter, Legal 등으로 변환 (비율 유지) |
| **페이지 교체** | 다른 PDF의 페이지로 교체 |
| **빈 페이지 제거** | 텍스트·이미지 없는 빈 페이지 삭제 |
| **중복 페이지 제거** | 내용 시그니처 기반 중복 정리 |
| **N-up 배치** | 2-up / 4-up 인쇄용 임포지션 |
| **자동 목차** | 제목 폰트 휴리스틱으로 북마크 생성 |

### 🔒 보안 및 보호
| 기능 | 설명 |
|------|------|
| **PDF 암호화** | AES-256 암호 설정 + 인쇄/복사/수정 등 권한 선택 |
| **PDF 복호화** | 암호 해제 |
| **문서 위생** | 메타데이터·첨부·주석 등 공유 전 스크럽 |
| **텍스트 워터마크** | 투명도·회전·위치 조절 |
| **이미지 워터마크** | 9개 위치·크기·투명도 조절 |
| **스탬프 추가** | 기밀, 승인됨, 사본, DRAFT 등 |

### 🔧 고급 편집
| 기능 | 설명 |
|------|------|
| **PDF 분할** | 페이지별 / 범위별 / **북마크 기준** 분리 |
| **PDF 압축** | `fast` / `compact` / `web` — 이미지 다운샘플·폰트 서브셋 포함 |
| **PDF 크롭** | 여백 자르기 또는 **콘텐츠 기준 자동 크롭** |
| **메타데이터 편집** | 제목, 저자, 주제, 키워드 수정 |
| **PDF 비교** | 텍스트 / **시각(픽셀)** / 둘 다 + 시각 diff PDF |
| **북마크 설정** | 목차 구조 직접 입력 |
| **양식 고정** | 채운 양식 필드를 영구 콘텐츠로 flatten |

### 📝 주석 및 마크업
| 기능 | 설명 |
|------|------|
| **텍스트 하이라이트** | 형광펜 효과 |
| **스티키 노트** | 메모 추가 |
| **주석 추가** | 텍스트 / 자유 텍스트 주석 |
| **밑줄 / 취소선** | 텍스트 마크업 |
| **도형 그리기** | 사각형, 원, 선 추가 |
| **하이퍼링크 추가** | URL / 페이지 이동 링크 |
| **텍스트 상자 삽입** | 좌표·프리셋·미리보기 **이동/리사이즈/인라인 편집** 후 삽입. **원본 바로 적용**, **큐 일괄 삽입**, 실험적 **영역 텍스트 교체**(redact+insert) 지원 |
| **텍스트 교정** | 검색어 기반 민감 정보 영구 삭제 |
| **영역 교정** | 좌표 직접 입력 **또는 미리보기 드래그 선택** 후 영구 삭제 |
| **배경색 추가** | 페이지 배경 변경 |
| **프리핸드 서명** | 손글씨 서명 삽입 |

### 📊 데이터 추출
| 기능 | 설명 | 출력 |
|------|------|------|
| **링크 추출** | 문서 내 URL 목록 | TXT |
| **이미지 추출** | 포함된 이미지 추출 | PNG / JPG |
| **SVG 내보내기** | 페이지별 벡터 SVG | SVG |
| **테이블 추출** | 표 데이터 추출 | CSV |
| **북마크 추출** | 목차 구조 추출 | TXT |
| **Markdown 변환** | `auto/native/text` 모드 | MD |
| **첨부파일 관리** | 첨부파일 추가 / 추출 | 다양한 형식 |

### 🤖 AI 기능 (Google Gemini)
| 기능 | 설명 |
|------|------|
| **PDF 요약** | 한국어 / 영어 요약 |
| **PDF 채팅** | PDF 내용에 대해 AI에게 질문 |
| **키워드 추출** | 핵심 키워드 자동 추출 |
| **요약 스타일** | 간결 / 상세 / 글머리 기호 선택 |

> AI 기능은 [Google AI Studio](https://aistudio.google.com/)에서 발급한 **Gemini API 키**와 `google-genai` 패키지가 필요합니다. AI 서비스 구현은 `src/core/ai/` 모듈에 있습니다.

### 🎨 UI/UX
- **다크 / 라이트 테마** — 글래스모피즘 디자인
- **줌 / 패닝 미리보기** — 마우스 휠 줌, 드래그 이동, 인쇄
- **미리보기 드래그 교정** — 고급 탭 영역 교정에서 미리보기에 사각형을 그려 좌표 자동 입력
- **미리보기 드래그 텍스트 삽입** — 편집 탭 텍스트 상자에서 드래그로 위치·크기를 지정하고 폰트/투명도 등 옵션 적용
- **미리보기 포커스 / 전체화면** — 좌측 패널을 접고 뷰어를 크게 보거나(`F11` 순환), OS 전체화면으로 확인 (`Ctrl+F11`)
- **미리보기 검색 / 북마크** — 접기·펼치기 가능한 사이드 패널 (`Ctrl+F`)
- **썸네일 그리드** — 모든 페이지 한눈에 보기
- **드래그 앤 드롭** — 파일 추가, 페이지 순서 변경
- **Undo / Redo** — 주요 편집 작업 실행 취소 / 다시 실행
- **진행 오버레이** — 작업 중 취소 가능한 풀스크린 다이얼로그
- **외부 파일 감지** — `QFileSystemWatcher`로 파일 변경을 감지하고 미리보기 자동 재로드
- **배치 처리** — 여러 파일에 동일 작업 일괄 적용 (워터마크 CJK·글자 크기/투명도)
- **환경설정** — 완료 알림(토스트/대화상자), 취소 시 대기 큐 비우기, 채팅 기록 디스크 저장 on/off
- **다국어 지원** — 한국어 / 영어 자동 감지 및 수동 설정

---

## 🚀 설치 및 실행

### 빌드된 실행 파일 사용 (Windows)

`dist/PDF_Master_v4.5.6.exe` 파일을 바로 실행하면 됩니다. 별도 설치 불필요.

### 소스에서 직접 실행

**요구 사항:** Python 3.10 이상

```bash
# 1. 의존성 설치 (pyproject.toml 기준)
pip install -e .[dev]
# 또는 호환 shim: pip install -r requirements-dev.txt

# AI 기능 사용 시 추가 설치
pip install -e .[ai]

# 2. 실행
python main.py
```

---

## 📖 사용 방법

### 1. PDF 병합
1. **병합** 탭 선택
2. 파일을 드래그 앤 드롭하거나 **파일 추가** 버튼 클릭
3. 목록에서 순서 조정 (드래그 이동 가능)
4. **병합 실행** 클릭 → 저장 위치 선택

### 2. PDF → 이미지 변환
1. **변환** 탭 선택
2. PDF 파일 선택
3. 출력 형식 선택 (PNG, JPG, WEBP 등)
4. DPI 설정 (기본: 150 / 고화질 권장: 200–300)
5. **변환 실행** → 출력 폴더 선택

### 3. 페이지 편집 (추출 / 삭제 / 회전)
1. **페이지** 탭 선택
2. PDF 파일 선택
3. 추출 / 삭제: 페이지 범위 입력 (예: `1-3, 5, 7-10`)
4. 회전: 각도 선택 후 적용 대상 선택
   - **전체 페이지**: 문서 전체 회전
   - **선택 페이지**: 썸네일에서 `Ctrl` / `Shift` 클릭으로 원하는 페이지만 선택 후 회전
5. **실행** → 저장 위치 선택

### 4. 워터마크 추가
1. **보안** 탭 선택
2. PDF 파일 선택
3. 워터마크 텍스트 입력
4. 투명도 / 글자 크기 / 회전 각도 / 위치(중앙 또는 타일) 설정
5. **워터마크 적용** 클릭

### 5. PDF 암호화 / 복호화
1. **보안** 탭 선택
2. PDF 파일 선택
3. 비밀번호 입력 (암호화) 또는 현재 비밀번호 입력 (복호화)
4. **암호화 실행** 또는 **복호화 실행** 클릭

### 6. AI 요약 및 채팅 (Gemini API)
1. **AI** 탭 선택
2. Gemini API 키 입력 후 **저장** 클릭 (최초 1회)
3. PDF 파일 선택
4. 기능 선택:
   - **요약**: 언어(한국어/영어)와 스타일(간결/상세/글머리 기호) 선택 → **요약 실행**
   - **채팅**: 질문 입력 → **질문하기**
   - **키워드**: **키워드 추출** 클릭

> **대화 삭제**는 현재 선택한 PDF의 기록만 초기화합니다.

### 7. Markdown 추출
1. **고급** 탭 → **추출** 섹션에서 **Markdown 추출** 선택
2. 추출 모드 선택 (`auto/native/text`)
3. 필요 시 YAML front matter, 페이지 마커, 이미지/표 placeholder 옵션 활성화
4. **Markdown 추출** 실행

### 8. 배치 처리
1. **배치** 탭 선택
2. 여러 PDF 파일 추가
3. 작업 선택 (압축, 워터마크, 암호화 등)
4. 공통 옵션 설정 후 **배치 실행** 클릭

### 언어 변경
메뉴 바 **Language** (🌐) → **Korean** 또는 **English** 선택 → 앱 재시작 후 적용

---

## ⌨️ 단축키

| 단축키 | 기능 |
|--------|------|
| `Ctrl+O` | 파일 열기 |
| `Ctrl+Q` | 앱 종료 |
| `Ctrl+T` | 다크 / 라이트 테마 전환 |
| `Ctrl+F` | 미리보기 검색 열기 |
| `F11` | 미리보기 포커스 ↔ 전체화면 ↔ 일반 순환 |
| `Ctrl+F11` | 미리보기 전체화면 (바로 진입 / 종료) |
| `Esc` | 텍스트 배치·영역 선택 취소 후 전체화면·포커스 단계 해제 |
| `Ctrl+Z` | 실행 취소 (Undo) |
| `Ctrl+Y` | 다시 실행 (Redo) |
| `Ctrl+1` | 병합 탭 |
| `Ctrl+2` | 변환 탭 |
| `Ctrl+3` | 페이지 탭 |
| `Ctrl+4` | 보안 탭 |
| `Ctrl+5` | 순서 변경 탭 |
| `Ctrl+6` | 배치 탭 |
| `Ctrl+7` | 고급 탭 |
| `Ctrl+8` | AI 탭 |

---

## ⚠️ 알려진 제한사항

| 항목 | 제한 |
|------|------|
| AI 요약 텍스트 | 최대 30,000자 |
| 이미지 렌더링 | 최대 8,000px |
| 파일 크기 | 최대 2GB |
| 암호화된 PDF | 일부 작업에서 사전 복호화 필요 |

---

## 📦 빌드 (PyInstaller)

소스에서 Windows 실행 파일을 직접 빌드하려는 경우:

```bash
# 빌드 도구 설치
pip install -e .[build]

# 정적 분석
python -m pyright

# 테스트 실행
python -m pytest -q

# 패키지 빌드
python -m build

# 실행 파일 빌드
python -m PyInstaller pdf_master.spec --clean

# 앱 초기화 smoke 확인
python main.py --smoke

# 패키지 smoke (clean PYTHONPATH)
powershell -ExecutionPolicy Bypass -File scripts/package_smoke.ps1
```

빌드 결과: `dist/PDF_Master_v4.5.6.exe` (~30–40MB)

타입 스텁은 `typings/` 디렉터리에 있으며 `pyrightconfig.json`에서 참조합니다.

검증 기준선: `python -m pytest -q` (opt-in Gemini smoke 1건 skip 가능). 기능 감사 SSOT는 `PROJECT_AUDIT.md`, 품질·아키텍처 감사는 `PROJECT_AUDIT_QUALITY.md`를 참고하세요.

---

## 📝 변경 이력

### v4.5.6
- **Deep compress**: `compact`/`web` 프로필에서 이미지 다운샘플·JPEG 재인코딩·폰트 서브셋
- **북마크 기준 분할**, 빈/중복 페이지 제거, 자동 목차, N-up(2/4), 문서 위생(sanitize)
- **스마트 크롭**(콘텐츠 bbox), **영역 교정**(좌표), **양식 flatten**
- **암호화 권한** UI (인쇄/복사/수정/주석/양식/조합)
- **PDF 비교** `text` / `visual` / `both` 모드 (스캔본 픽셀 차이 탐지)
- **SVG 페이지 내보내기**, PyMuPDF 1.28+ `linear` 미지원 시 저장 폴백
- Worker 도메인 `cleanup_ops` 추가 및 회귀 테스트 확장
- **2026-07-15 감사 후속**: AI 작업 취소(`cancel_check`)·암호화 PDF 미리보기 암호 재사용, 빈 페이지 보수 판정, visual compare 오류 상태, 영역 교정 확인 다이얼로그, 배치/추출 cancel·i18n 보강
- **2026-07-21 SOLID 코드 분할**: Worker 도메인 패키지(`annotation`/`extract`/`cleanup`/`page`/`transform`/`compare`) + settings/constants/undo 구현 패키지 + progress UI 분리. public import·동작 계약 유지
- **2026-07-22 감사 후속**: 썸네일 로더 sender 가드, AI temp/atomic orphan 스윕(`temp_cleanup`), AI retry 분할 sleep+취소 비재시도, blank/dedupe/sanitize 확인 다이얼로그, 취소 롤백 mtime 휴리스틱 제거, `list_annotations` text 계약, 배치 암호 권한 안내, chat session single-flight
- **2026-07-27 감사 후속**: 기능 감사 SSOT를 `PROJECT_AUDIT.md`로 통일, 암호 PDF AI UI(미리보기 암호 재사용), merge 0페이지 fail-fast, AI chat `cancel_check` 전파, 배치 암호 권한 체크박스, blank/dedupe dry-run 카운트, **미리보기 드래그 영역 교정**
- **2026-07-31**: **미리보기 드래그 텍스트 삽입** — `insert_textbox`에 위치 드래그·W/H·CJK/Base-14 폰트·투명도·90° 회전·정렬·레이어 옵션, redact/textbox 드래그 타겟 격리
- **2026-08-03**: **미리보기 포커스/전체화면** (`F11` 순환, `Ctrl+F11`), 텍스트 상자 리사이즈·인라인 편집·원본 바로 적용·큐 일괄 삽입·실험적 영역 교체; 감사 후속(실패 플래그 리셋, busy 가드, redact hard-fail, 큐 경로 고정)
- **2026-08-04 감사 후속**: 배치 워터마크 CJK·옵션(글자 크기/투명도), 페이지 범위 엄격 검증, 페이지 밖 텍스트 상자 hard-fail, AI 채팅 HTML 이스케이프, 채팅 디스크 저장/완료 알림/취소 시 대기 큐 환경설정, 비교 스크롤 리포트, OCR 텍스트 추출 옵션(`use_ocr`, Tesseract), AI 임시 평문 ACL·스트림 취소 보강; `PROJECT_AUDIT.md` 갱신
- **2026-08-05 SOLID Round 2**: Worker 잔여 도메인(`ai`/`batch`/`compose`/`form`/`security`/`_pdf_helpers_impl`) 패키지화; UI textbox_impl·tab section builders·thumbnail grid mixins·preview interaction 분할; `PreviewWidgetHost`/`ThumbnailGridHost`; public import·mode/kwargs 불변. 설계: `docs/superpowers/specs/2026-08-05-code-split-solid-round2-design.md`
- **2026-08-05 감사 잔여 후속**: 채팅 partial HTML 이스케이프·취소 중 partial 차단; `_is_pdf_encrypted` 삼상 복구; OCR 0성공 hard-fail; AI 요약/채팅 공용 스트림 cancel+close; 강제 종료 스윕 로그; 채팅 디스크 저장 프라이버시 안내; AI temp ACL 경고; 첨부 100MB 상한; 썸네일 로더 wait 보강; `tests/test_audit_2026_08_05_followup.py`
- **2026-08-05 품질 감사(Track B) 후속**: AI 텍스트 캐시 종료 청소·closeEvent 연동; Worker/pending 민감 kwargs scrub; `get_pdf_info` i18n; PyMuPDF 미설치 기동 안내; 대용량 Undo 백업 스킵(200MB); FALLBACK 키 동기 스모크; structure budget 본체 가드; `PROJECT_AUDIT_QUALITY.md` / `tests/test_audit_2026_08_05_quality_followup.py`
- **2026-08-05 품질 Track B 3단계**: AI Worker 도메인 분할; compare 픽셀 헬퍼 추출; 썸네일 pixmap LRU; monkeypatch 계약 SSOT(`src/ui/contracts`); `tests/test_monkeypatch_contracts.py`, `tests/test_thumbnail_pixmap_lru.py`

### v4.5.5
- 미리보기 줌·패닝·인쇄 안정화 (Qt 인쇄 파이프라인)
- 암호화 PDF 썸네일 로딩 시 미리보기 비밀번호 세션 재사용
- Undo/Redo 범위 확장 (서명, 하이라이트, 스티키 노트, 페이지 복사 등)
- PDF 비교 — 줄 순서 변경 및 중복 감지 + 양방향 시각 diff PDF
- AI 요약 결과에 소스 정보 및 텍스트 제한 초과 여부 표시
- 출력 대화상자가 마지막 출력 폴더를 기억하고 재사용
- 배치 출력 파일명 충돌 자동 회피 (`__2`, `__3` 접미사)
- 외부 프로그램이 PDF를 덮어써도 미리보기 자동 재로드
- `페이지 크기 변경` — 원본 비율 유지 fit-center 배치
- Markdown 추출 모드 추가 (`auto / native / text`, front matter, 페이지 마커)
- AI 채팅 **대화 삭제**가 현재 PDF 기록만 초기화
- 배치 미지원 operation / option 누락 fail-fast (silent copy 제거)
- Worker 대기 큐 FIFO(`_pending_workers`), busy 중 단축키·파일 열기 비활성화
- `remove_annotations` 취소 체크, 빈 `search_term` reject, 전역 예외 i18n

### v4.5.4
- 회전 섹션 전용 썸네일 목록 추가
- 선택한 페이지만 부분 회전 지원 (`Ctrl` / `Shift` 다중 선택)
- 페이지 회전 시 우측 미리보기 즉시 동기화

### v4.5.3
- `replace_page`, `set_bookmarks`, `add_annotation` UI 노출
- 배치 워터마크 안정화 및 파일별 실패 원인 요약
- 첨부파일 추출 보안 강화 (파일명 정규화, 경로 탈출 차단)

### v4.5.2
- 프리핸드 서명 UI 추가
- PDF → 이미지 출력 형식 확장 (WEBP, BMP, TIFF 추가)
- 주석 입력값 검증 강화

### v4.5
- 도형 그리기 (사각형, 원, 선)
- 하이퍼링크 추가 (URL / 페이지 이동)
- 텍스트 상자 삽입
- 다른 PDF에서 페이지 복사
- AI PDF 채팅 및 키워드 추출

### v4.4
- 한국어 / 영어 다국어 지원 (시스템 언어 자동 감지)
- 언어 수동 변경 기능

### v4.3
- Undo / Redo 기능
- 취소 가능한 진행 오버레이

### v4.2
- Google Gemini API (google-genai) 적용
- 빌드 경량화 (~30–40MB)

---

## 📄 라이선스

MIT License — Copyright (c) 2026 PDF Master
